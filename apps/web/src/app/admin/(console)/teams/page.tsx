...
import { RecordEditor, MemberRoleEditor } from "@/components/admin/RecordEditors";
...
type Team = { themeColor?: string; id: string; name: string; slug: string; status: string; ownerId: string; ownerName: string; memberCount: number; itemCount: number; activeShareCount?: number; createdAt: string; updatedAt: string; lastActivityAt?: string | null; canRestore?: number; eligibleOwnerCount?: number };
...
export default function TeamsPage() {
  ...
  async function openTeam(team: Team) {
    ...
    try {
      ...
      if (detailRequest.current === requestId) { setDetail(result); setSelected(result.team); }
    } catch (reason) {
  ...
  return <>
    ...
    {selected && <div className="admin-drawer-layer" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) { detailRequest.current += 1; setSelected(null); setDetail(null); } }}><aside className="admin-drawer" aria-label={`${selected.name} 团队详情`}><header><div><span className="admin-team-avatar large">{selected.name.slice(0, 1)}</span><div><small>团队详情</small><h2>{selected.name}</h2></div></div><button type="button" onClick={() => { detailRequest.current += 1; setSelected(null); setDetail(null); }} aria-label="关闭"><X size={19} /></button></header>{detailLoading ? <LoadingState label="加载团队详情" /> : detail ? <div className="admin-drawer-body">
      <RecordEditor key={detail.team.id} kind="teams" record={detail.team} onSaved={() => { setNotice("团队信息已更新"); reload(); void openTeam(detail.team); }} /><section className="admin-detail-summary"><div><span>运行状态</span><StatusBadge status={detail.team.status} /></div><div><span>团队所有者</span><b>{detail.team.ownerName}</b></div><div><span>创建时间</span><b>{formatDate(detail.team.createdAt)}</b></div><div><span>内容 / 成员</span><b>{detail.team.itemCount} / {detail.team.memberCount}</b></div></section>
      <section><div className="admin-section-title"><h3>成员与角色</h3><span>{detail.members.length} 人</span></div><div className="admin-member-list">{detail.members.map((member) => <div key={member.userId || member.id}><span className="admin-user-avatar">{member.nickname.slice(0,1)}</span><span><b>{member.nickname}</b><small>加入于 {formatDate(member.joinedAt, false)}</small></span>{member.role === 'owner' || (member.userId || member.id) === detail.team.ownerId || member.status !== 'active' ? <span className={`admin-role ${member.role}`}>{member.role}</span> : <MemberRoleEditor key={`${member.userId || member.id}-${member.role}`} teamId={detail.team.id} userId={(member.userId || member.id)!} role={member.role} onSaved={() => { reload(); void openTeam(detail.team); }} />}</div>)}</div></section>
